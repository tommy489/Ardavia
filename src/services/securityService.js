  ...
  try {
    await ch.send({
      embeds: [new EmbedBuilder()
        .setColor(color)
        .setTitle('🛡️  Security — Event Detected')
        .setDescription(description)
        .setFooter({ text: 'Ardavia Council • Security' })
        .setTimestamp()
      ]
    });
  } catch (e) {
  ...
  try {
    switch (action) {
      ...
      case 'warn': {
        ...
        try { await member.send(`⚠️ **Warning [${count}/3]** — ${reason} — Ardavia Council`); } catch (_) {}
        if (count >= 3) {
...
const checkMessage = async (message) => {
  ...
  if (cfg.antimention_enabled) {
    ...
    if (cnt >= cfg.antimention_threshold) {
      const ok = await performAction(member, 'Mass mention', cfg.antimention_action, message.guild, cfg);
      if (ok) {
  ...
  if (cfg.antispam_enabled) {
    ...
    if (rec.count >= cfg.antispam_threshold) {
      ...
      const ok = await performAction(member, 'Message spam', cfg.antispam_action, message.guild, cfg);
      if (ok) {
        await sendLog(message.guild, cfg,
          `**⚠️ Anti-Spam** — ${message.author} (\`${message.author.tag}\`)\n${rec.count} messages in ${cfg.intervalSeconds}s → Action: \`${cfg.antispam_action}\``
        );
      }
    }
  }
...
const checkJoin = async (member) => {
  ...
  if (cfg.antibot_enabled && member.user.bot) {
    const ok = await performAction(member, 'Unauthorised bot', cfg.antibot_action, guild, cfg);
  ...
  if (cfg.antiraid_enabled) {
    ...
    if (rec.count >= cfg.antiraid_threshold) {
      ...
      const ok = await performAction(member, 'Raid detected', cfg.antiraid_action, guild, cfg);
      if (ok) await sendLog(guild, cfg,
        `**🚨 Anti-Raid** — ${member.user.tag} (${member.id})\n${rec.count} joins in ${cfg.intervalSeconds}s → Action: \`${cfg.antiraid_action}\``
      );
    }
  }
};
